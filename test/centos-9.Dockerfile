FROM --platform=linux/amd64 quay.io/centos/centos:stream9 AS opam-builder
RUN dnf config-manager --set-enabled crb && yum makecache && yum install -y --allowerasing gcc gcc-c++ make patch unzip bzip2 tar git curl openssl sudo diffutils findutils libcap-devel
RUN git clone --depth 1 --branch 2.4.1 https://github.com/ocaml/opam.git /tmp/opam
WORKDIR /tmp/opam
RUN make cold
RUN make install

FROM --platform=linux/amd64 quay.io/centos/centos:stream9 AS day10-install-builder
RUN dnf config-manager --set-enabled crb && yum makecache && yum install -y --allowerasing gcc gcc-c++ make patch unzip bzip2 tar git curl diffutils findutils bubblewrap
