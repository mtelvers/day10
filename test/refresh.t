...
idle window rather than only when something is known to be stale.
  ...
  $ mkdir -p cache/debian-13-x86_64/base/fs
...
unnoticed is the one nobody thought to name.
  ...
  >   mkdir -p cache/$p/base/fs && touch cache/$p/base/build.log
  > done
  $ refresh --max-age 24
...
Whatever else is in the cache is not a platform.  A cache on a filesystem of
its own has a lost+found, and by name alone that reads like one; having no
base/fs is what rules it out.

  $ mkdir -p cache/lost+found cache/temp-abc123
  $ refresh --max-age 24 2>&1 | grep -c "lost+found\|temp-"
  0
  [1]

A platform whose name will not come apart is left alone rather than guessed at,
since there is no telling which distribution's index to update.

  $ mkdir -p cache/nonsense/base/fs
  $ touch cache/nonsense/base/build.log
  $ refresh --os-distribution nonsense
  [WARNING] No base image to refresh in cache
